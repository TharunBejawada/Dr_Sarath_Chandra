  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const testiRes = await fetch(`${API_URL}/api/testimonials`);
        const testiData = await testiRes.json();
        const testiCount = testiData ? testiData.length : 0;

        setStats(prev => ({ ...prev, testimonials: testiCount }));
      } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
         ...
         <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col justify-between h-40 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <HeartPulse className="w-24 h-24 text-green-600" />
            </div>

            <div className="flex items-start justify-between relative z-10">
                <div>
                    <h3 className="text-slate-500 text-sm font-bold uppercase tracking-wide">Testimonials</h3>
                    <div className="mt-2">
                        {loading ? (
                            <Loader2 className="w-8 h-8 animate-spin text-slate-300" />
                        ) : (
                            <p className="text-4xl font-bold text-slate-900">{stats.testimonials}</p>
                        )}
                    </div>
                </div>
            ...
            <Link href="/admin/testimonials" className="text-sm font-bold text-green-600 flex items-center gap-1 mt-auto relative z-10 hover:underline">
                Manage Testimonials <ArrowUpRight className="w-4 h-4" />
            </Link>
         </div>
