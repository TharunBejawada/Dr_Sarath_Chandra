...
const ImageUploader = ({ label, value, onChange }: { label: string, value: string, onChange: (url: string) => void }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/blogs/uploadblogImage`, { method: "POST", body: formData });
...
export default function ModifyService({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/services/getServiceById/${id}`)
      ...
      .then(data => {
         if(data.Item) {
             // Safe merge with defaults to avoid null pointer errors on arrays
             setService(prev => ({ 
                ...prev, 
                ...data.Item, 
                conditions: data.Item.conditions || [],
                treatments: data.Item.treatments || [],
                faqs: data.Item.faqs || [],
                benefits: { 
                    ...prev.benefits, 
                    ...(data.Item.benefits || {}),
                    list: data.Item.benefits?.list || []
                },
                whyChoose: {
                    ...prev.whyChoose,
                    ...(data.Item.whyChoose || {})
                ...
                backlinkSchema: data.Item.backlinkSchema || []
             }));
         }
      })
      .catch(err => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/services/updateService/${id}`, {
        method: "PUT",
  ...
  return (
    <div className="max-w-7xl mx-auto pb-40">
      <div className="flex items-center justify-between mb-8 sticky top-0 z-10 bg-slate-50 py-4 shadow-sm px-4 -mx-4">
         <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-slate-900">Modify Service</h1>
         </div>
         <button onClick={handleSubmit} disabled={isSubmitting} className="bg-blue-600 text-white px-8 py-3 rounded-lg font-bold flex items-center gap-2 hover:bg-blue-700 shadow-lg transition">
            <Save className="w-5 h-5" /> {isSubmitting ? "Updating..." : "Update Service"}
         </button>
      </div>
           ...
           <div className={sectionClass}>
               ...
               <div className="space-y-4 mb-8">
                  ...
                  <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                     ...
                     {service.benefits?.list?.map((benefit: any, i: number) => (
